...
- P11 now has a frozen 13-repository compatibility corpus selected before further tuning, with exact repository/lockfile/license/source pins, CI online pin verification, explicit unsupported classes, separate FS/module/process/HTTP/package/watch reporting and a machine-readable baseline generated from the tested corpus; the first pinned real repository (`sindresorhus/yoctocolors`) executes through Workspace VFS and native ESM in the isolated browser Worker with bounded `node:tty` semantics.
...
3. Broader real-repository execution beyond the frozen 13-repository P11 corpus and first `yoctocolors` browser pass; npm package-lock preflight is verified, but pnpm/yarn, native-addon, generic watcher, repeated-browser and published-package certification remain open.
